...
def main() -> None:
    ...
    env_cfg = EnvConfig()
    agent   = DDPGAgent(DDPGConfig(device=args.device))

    print("\n========== TRAIN DDPG ==========")
    scores, sr, cr = train(agent, env_cfg, args.episodes, "DDPG")

    # Lưu checkpoint
    agent.save("checkpoints/ddpg_actor.pt", "checkpoints/ddpg")

    # Lưu kết quả để plot_results.py đọc
    result = {"scores": scores, "success_rates": sr, "collision_rates": cr}
    out_path = f"{args.outdir}/ddpg_results.json"
    with open(out_path, "w") as f:
        json.dump(result, f)

    print(f"\nDONE DDPG! Saved → {out_path}")
