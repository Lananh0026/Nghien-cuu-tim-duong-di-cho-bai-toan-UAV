...
def main() -> None:
    ...
    # Generalization test như Fig. 9 trong paper
    with open(os.path.join(args.outdir, "fig9_generalization.txt"), "w", encoding="utf-8") as f:
        for n_agents in range(2, 13):
            sr, cr = evaluate(agent, n_agents, episodes=100)
            line = f"n_agents={n_agents}, success_rate={sr:.6f}, collision_rate={cr:.6f}\n"
            print(line.strip())
            f.write(line)
